...
exports.event = 'plugin:guild:renameretag';
exports.description = 'Rename/retag from your guild.';
exports.args = 'name, tag';
exports.socket = (socket, primus, respond) => {
    const request = ({ name, tag }) => __awaiter(this, void 0, void 0, function* () {
        if (!socket.authToken)
            return;
        ...
        if (!player || !player.guild || player.guild.$noGuild)
            return;
        logger_1.Logger.info('Socket:Guild:RenameRetag', `${playerName} (${socket.address.ip}) retagging guild ${name} [${tag}].`);
